...
public class AppController {
    ...
    /*
    de end-points hieronder zijn bedoeld om een JSON-Array op te vangen deze zien er als volgt uit:
=======================
    [
        {
            "name": "Het avondeten",
...
==================================
Dit zijn 2 boeken met hetzelfde author, alleen sturen we dus geen ID, deze wordt gechecked in AppService.java
     */

    @JsonFormat(with = JsonFormat.Feature.ACCEPT_SINGLE_VALUE_AS_ARRAY)
    @PostMapping("/books")
    private void addMultipleBooks(@RequestBody List<Book> books)
    {
        appService.multiNewBook(books);
    }

    @JsonFormat(with = JsonFormat.Feature.ACCEPT_SINGLE_VALUE_AS_ARRAY)
    @PostMapping("/authors")
    private void addMultipleAuthors(@RequestBody List<Author> authors)
    {
        appService.multiNewAuthor(authors);
    }
